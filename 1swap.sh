...
# 添加 SWAP
add_swap() {
    echo -e "${Green}请输入需要添加的 swap 大小（MB），建议为内存的 2 倍：${Font}"
    ...
    if [[ "$ROOT_FS" == "btrfs" ]]; then
        if ! command -v btrfs >/dev/null 2>&1; then
            echo -e "${Red}btrfs-progs 未安装，请先安装 btrfs-progs${Font}"
            exit 1
        fi
        btrfs filesystem mkswapfile --size ${swapsize}M $SWAPFILE
    else
        if ! fallocate -l "${swapsize}M" $SWAPFILE 2>/dev/null; then
            echo -e "${Red}fallocate 不可用，使用 dd 创建...${Font}"
